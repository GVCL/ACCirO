...
def detect_text(image, check_slope = 0):
    # load net
    ...
    if check_slope != 0:
        for i in range(len(bboxes)):
           if(slope[i]!='_' and int(slope[i])!= 0):
                # print(slope[i])
                # d1 = pytesseract.image_to_data(rotate(cv2.imread(ime_name,0), slope[i], cval=255), output_type=Output.DICT, lang='eng')
                # d2 = pytesseract.image_to_data(rotate(cv2.imread(ime_name,0), -slope[i], cval=255), output_type=Output.DICT, lang='eng')
                # print(d1['text'],d2['text'],"\n--------")
                ime_name="Deep_TextRecognition/Detected_Images/"+str(i)+".png"
                cv2.imwrite(ime_name,rotate(cv2.imread(ime_name,0), check_slope*slope[i], cval=255))
